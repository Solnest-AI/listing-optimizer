...
2. **No write-back.** Never push to Hospitable / Airbnb / VRBO. Hospitable calls are **read-only** (`hospitable_get_property`, `hospitable_get_property_images`, `hospitable_list_reviews`, `hospitable_get_property_calendar`, `hospitable_list_reservations`). **Never** call `hospitable_update_property_calendar`, `hospitable_update_reservation`, `hospitable_create_reservation`, or any create/update/delete. Output is paste-ready only.
...
### 1.6. Occupancy — Hospitable (SOURCE OF TRUTH) + RankBreeze cross-check
Hospitable's live calendar is the authoritative occupancy record; RankBreeze occupancy is a scraped estimate kept only as a cross-check.
...
## The listings — DISCOVERED from the connected PMS (never hardcoded)
This tool ships with **no listings**. At run time, enumerate whatever the user has connected — these belong to whoever connected their PMS:
1. `hospitable_list_properties(include="listings")` → the account's properties (id, name, public_name, and `listings[]` with the Airbnb `platform_id`).
2. The user names a listing (or "all"); match by name. Derive a **slug** = lowercase, hyphenated name (e.g. "The Beach House" → `beach-house`).
3. For the chosen property, `hospitable_get_property(id, include="listings")` → full detail: `address.coordinates` (lat/lng), `capacity` (bedrooms/beds/bathrooms/max), `amenities`, the Airbnb `platform_id`. **Use these** for the subject + `pull_comps` coords/sizes — never a hardcoded registry.
